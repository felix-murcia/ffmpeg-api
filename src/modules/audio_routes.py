    ...
    def validate_audio():
        ...
        try:
            ...
            if duration < 1:
                ...
                warnings.append("El audio es muy largo (> 1 hora), puede tardar mucho en transcribirse")
            elif duration > 7200:  # 2 horas
                issues.append("El audio es demasiado largo (> 2 horas) para transcripción")
            
            # Verificar sample rate
            if sample_rate < 8000:
                issues.append(f"Frecuencia de muestreo demasiado baja ({sample_rate} Hz). Mínimo 8000 Hz")
            elif sample_rate < 16000:
                warnings.append(f"Frecuencia de muestreo baja ({sample_rate} Hz). Se recomienda 16000 Hz o más")
            
            # Verificar canales
            if channels == 0:
                issues.append("No se detectaron canales de audio")
            elif channels > 2:
                warnings.append(f"Audio con {channels} canales. Se convertirá a mono para transcripción")
            
            # Verificar bitrate (si está disponible)
            if bitrate > 0 and bitrate < 32000:
                warnings.append(f"Bitrate bajo ({bitrate//1000} kbps). La calidad podría ser insuficiente")
            ...
            if mean_volume is not None:
                if mean_volume < -30:
                    issues.append(f"Volumen muy bajo ({mean_volume:.1f} dB). El audio apenas se escucha")
                elif mean_volume < -20:
                    warnings.append(f"Volumen bajo ({mean_volume:.1f} dB). Puede afectar la transcripción")
                elif mean_volume > -5:
                    warnings.append(f"Volumen muy alto ({mean_volume:.1f} dB). Puede haber distorsión")
            
            # Verificar silencios excesivos
            if silence_detected and silence_duration > (duration * 0.5):
                issues.append(f"Demasiado silencio en el audio ({silence_duration:.1f}s de {duration:.1f}s)")
            elif silence_detected and silence_duration > (duration * 0.3):
                warnings.append(f"Mucho silencio en el audio ({silence_duration:.1f}s de {duration:.1f}s)")
            ...
            if codec not in ['mp3', 'wav', 'opus', 'aac', 'flac']:
                recommendations.append(f"El códec {codec} puede no ser óptimo. Se recomienda convertir a WAV o MP3")
            
            if sample_rate != 16000:
                recommendations.append("Se recomienda convertir a 16kHz para mejor rendimiento de transcripción")
            ...
            return jsonify({
                ...
                "metadata": {
                    ...
                    "silence_percentage": round((silence_duration / duration) * 100, 1) if duration > 0 else 0,
                    "has_voice": has_voice
                },
                "suggested_conversion": {
                    ...
                    "target_channels": 1,
                    "command": "ffmpeg -i input.wav -ac 1 -ar 16000 output.wav" if not is_optimal else None
                }
            })
...
def validate_audio_from_path_impl(file_path):
    ...
    try:
        ...
        quality_result = subprocess.run(quality_check_cmd, capture_output=True, text=True, timeout=30)
